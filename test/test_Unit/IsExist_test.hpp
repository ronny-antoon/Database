#ifndef UNIT_IS_EXIST_TEST_HPP
#define UNIT_IS_EXIST_TEST_HPP
...
// setup test suite
class IsExistTest : public ::testing::Test
{
protected:
    int _startFreeHeap;
    int _endFreeHeap;
...
/** Testing isExist Method of DatabaseAPI class
...
TEST_F(IsExistTest, DATABASE_OK)
{
    // arrange
    const char *key = "key";
    ...
    EXPECT_CALL(*mockNVSDelegate, get_str(::testing::_, testing::StrEq(key), nullptr, ::testing::NotNull()))
        .WillOnce(::testing::DoAll(::testing::SetArgPointee<3>(5), ::testing::Return(NVSDelegateError_t::NVS_DELEGATE_OK)));
...
TEST_F(IsExistTest, DATABASE_KEY_INVALID)
{
    // arrange
    const char *key = "";

    // act
    DatabaseError_t err1 = databaseAPI->isExist(key);
    DatabaseError_t err2 = databaseAPI->isExist(nullptr);
    // assert
    EXPECT_EQ(err1, DatabaseError_t::DATABASE_KEY_INVALID);
    EXPECT_EQ(err2, DatabaseError_t::DATABASE_KEY_INVALID);
}

TEST_F(IsExistTest, DATABASE_KEY_NOT_FOUND)
{
    // arrange
    const char *key = "key";
...
TEST_F(IsExistTest, DATABASE_ERROR)
{
    // arrange
    const char *key = "key";
